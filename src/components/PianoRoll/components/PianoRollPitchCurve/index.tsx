...
const PianoRollPitchCurve: React.FC<PianoRollPitchCurveProps> = ({ note, ...other }) => {
  ...
  const drawLine = (x1: number, x2: number, y: number) => {
    return <line x1={x1} y1={y} x2={x2} y2={y} stroke={theme.curve.pitchBendCurveColor} strokeWidth={theme.curve.pitchBendCurveLineWidth} />;
  };
  ...
  const drawVibratoCurveLine = (
    ...
    return <path d={`M ${x1} ${y} ${pathData.join(' ')}`} fill="none" stroke={theme.curve.pitchBendCurveColor} strokeWidth={theme.curve.pitchBendCurveLineWidth} />;
  };
  ...
  return (
    <svg aria-label="piano-roll-pitch-curve"
         width={laneLength}
         height={canvasHeight}
