...
export default function usePianoRollMouseHandlers() {
  ...
  const [mouseHandlerMode, setMouseHandlerMode] = useState(
    PianoRollLanesMouseHandlerMode.None,
  );
  ...
  const getTickAndNoteNumFromEvent = (e: PointerEvent) => {
    ...
    const ticks = pianoRollStore.roundDownTickToNearestGrid(
      pianoRollStore.getTickFromEvent(e),
    );
  ...
  const onPointerDown: React.PointerEventHandler = (event) => {
    ...
    const setMouseHandlerModeForNote = () => {
      if (
        pianoRollStore.isNoteLeftMarginClicked(
          noteClicked!,
          event.nativeEvent.offsetX,
          event.nativeEvent.offsetY,
        )
        ...
        pianoRollStore.isNoteRightMarginClicked(
          noteClicked!,
          event.nativeEvent.offsetX,
          event.nativeEvent.offsetY,
        )
    ...
    if (noteClicked) {
      ...
      if (event.metaKey) {
        ...
        const selectionTicks = pianoRollStore.getTickFromOffsetX(
          event.nativeEvent.offsetX,
        );
        dispatch({
  ...
  const onPointerMove: React.PointerEventHandler = (event) => {
    console.log(
      `pointer move ${PianoRollLanesMouseHandlerMode[mouseHandlerMode]} ${event.nativeEvent.offsetX} ${event.nativeEvent.offsetY}`,
    );
    // const {deltaPitch, deltaTicks, deltaY, deltaX} = calculateDeltas(event.nativeEvent)
    const deltaY =
      event.nativeEvent.offsetY - pianoRollStore.noteModificationBuffer.initY;
    const deltaX =
      event.nativeEvent.offsetX - pianoRollStore.noteModificationBuffer.initX;
    const deltaTicks = pianoRollStore.getTickFromOffsetX(deltaX);
    const deltaPitch =
      pianoRollStore.getNoteNumFromOffsetY(event.nativeEvent.offsetY) -
      pianoRollStore.getNoteNumFromOffsetY(
        pianoRollStore.noteModificationBuffer.initY,
      );
    switch (mouseHandlerMode) {
      ...
      case PianoRollLanesMouseHandlerMode.NotesTrimming: {
        const newNotes =
          pianoRollStore.noteModificationBuffer.notesSelected.map(
            (bufferedNote) => ({
              ...bufferedNote,
        ...
        const newNotes =
          pianoRollStore.noteModificationBuffer.notesSelected.map(
            (bufferedNote) => ({
              ...
              noteNumber: Math.min(
                127,
                Math.max(0, bufferedNote.noteNumber + deltaPitch),
              ),
              tick: bufferedNote.tick + deltaTicks,
            }),
          );
        ...
        const newNotes =
          pianoRollStore.noteModificationBuffer.notesSelected.map(
            (bufferedNote) => ({
              ...
              velocity: Math.max(
                1,
                Math.min(127, bufferedNote.velocity - deltaY / 3),
              ),
            }),
          );
        dispatch({ type: "MODIFYING_NOTES", payload: { notes: newNotes } });
  ...
  const onDoubleClick: React.MouseEventHandler = (event) => {
    const noteClicked = pianoRollStore.getNoteFromPosition(
      event.nativeEvent.offsetX,
      event.nativeEvent.offsetY,
    );
    if (noteClicked === null) {
  ...
  const onWheel: React.WheelEventHandler = (event) => {
    ...
    console.log(
      "calculated width",
      pianoRollStore.pianoLaneScaleX * pianoRollStore.laneLength,
    );
    ...
    if (event.ctrlKey) {
      ...
      if (
        transform.pianoLaneScaleX *
          (1 + event.deltaY * 0.01) *
          transform.laneLength >
        componentRef!.clientWidth
      ) {
  ...
  const updateCursorStyle = (e: PointerEvent) => {
    ...
    const noteHovered = pianoRollStore.getNoteFromPosition(
      e.offsetX,
      e.offsetY,
    );
