...
export function addNote(state: PianoRollStore, action: AddNoteAction) {
  const newNote = createNote(
    state,
    action.payload.ticks,
    action.payload.noteNum,
  );
  return {
    ...state,
...
export function modifyingNotes(
  state: PianoRollStore,
  action: ModifyingNotesAction,
) {
  ...
  const notesNotModified = state.pianoRollNotes.filter(
    (note) => !notesIdsToBeModified.includes(note.id),
  );
...
export function deleteSelectedNotes(
  ...
  const notesToBeDeleted = state.pianoRollNotes.filter(
...
export function toggleSelectedNoteVibratoMode(
  state: PianoRollStore,
  action: ToggleSelectedNoteVibratoModeAction,
) {
  return {
    ...state,
    pianoRollNotes: state.pianoRollNotes.map((note) => ({
      ...note,
      vibratoMode: note.isSelected
        ? (note.vibratoMode + 1) % 2
        : note.vibratoMode,
    })),
  };
}
...
export function vibratoDepthDelayChangeSelectedNote(
  ...
  return {
    ...
    pianoRollNotes: state.pianoRollNotes.map((note) => {
      if (note.isSelected) {
        const newVibratoDepth =
          note.vibratoDepth + 0.6 * action.payload.depthOffset;
        const newVibratoDelay =
          note.vibratoDelay - 4 * action.payload.delayOffset;
        return {
          ...note,
          vibratoDelay: Math.max(
            Math.min(newVibratoDelay, note.duration * 0.9),
            0,
          ),
...
export function vibratoRateChangeSelectedNote(
  state: PianoRollStore,
  action: VibratoRateChangeSelectedNoteAction,
) {
  return {
    ...state,
    pianoRollNotes: state.pianoRollNotes.map((note) => {
      if (note.isSelected) {
        const newVibratoRate =
          note.vibratoRate - 0.15 * action.payload.rateOffset;
...
export function setNoteInMarqueeAsSelected(
  state: PianoRollStore,
  action: SetNoteInMarqueeAsSelectedAction,
) {
...
export function moveNoteAsLatestModified(
  state: PianoRollStore,
  action: MoveNoteAsLatestModifiedAction,
) {
  return {
    ...state,
